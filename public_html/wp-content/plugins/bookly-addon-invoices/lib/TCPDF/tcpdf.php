...
class TCPDF {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($file[0] === '@') {
            ...
            } elseif (($imsize = TCPDF_IMAGES::booklyGetImageSize($file)) === FALSE && strpos($file, '__tcpdf_'.$this->file_id.'_img') === FALSE){
		...
		if (!empty($imgdata)) {
			...
			fclose($fp);
			unset($imgdata);
			$imsize = TCPDF_IMAGES::booklyGetImageSize($file);
			if ($imsize === FALSE) {
				unlink($file);
				$file = $original_file;
			}
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		if (($parsed === false) AND function_exists('imagecreatefrompng')) {
			try {
				// generate images
				$img = imagecreatefromstring(TCPDF_IMAGES::booklyFileGetContents($file));
