...
class SB_Instagram_Feed {
			...
			foreach ( $terms as $term_and_params ) {
				...
				if ( ! isset( $term_and_params['error'] )
					...
					if ( ! $connection->is_wp_error() && ! $connection->is_instagram_error() ) {
						...
						if ( $this->can_try_another_request( $type, $connected_accounts_for_feed[ $term ], 0 ) ) {
	 ...
	 * @since 2.0/5.1.1
	 */
	protected function can_try_another_request( $type, $connected_account_with_error, $attempts ) {
		return false;
	}
