...
class SB_Instagram_Feed_Pro extends SB_Instagram_Feed {
	...
	public function need_avatars( $settings ) {
		if ( isset( $settings['type'] ) && $settings['type'] === 'hashtag' ) {
			return false;
		} elseif ( isset( $settings['disablelightbox'] ) && ( $settings['disablelightbox'] === 'true' || $settings['disablelightbox'] === 'on' || $settings['disablelightbox'] === true ) ) {
			return false;
		} else {
			return true;
		}
	...
	protected function filter_posts( $post_set, $settings = array() ) {
		...
		if ( empty( $settings['includewords'] ) && empty( $settings['excludewords'] ) && empty( $settings['whitelist'] ) && empty( $settings['hidephotos'] ) && $settings['media'] === 'all' ) {
			return $post_set;
		}
	 ...
	 * @since 5.0
	 */
	protected function feed_is_complete( $settings = array(), $offset = 0 ) {
	...
	protected function add_other_atts( $other_atts, $settings ) {
		...
		if ( $layout === 'carousel' ) {
			$arrows                      = $settings['carouselarrows'] === 'true' || $settings['carouselarrows'] === 'on' || $settings['carouselarrows'] === 1 || $settings['carouselarrows'] === true;
			$pag                         = $settings['carouselpag'] === 'true' || $settings['carouselpag'] === 'on' || $settings['carouselpag'] === 1 || $settings['carouselpag'] === true;
			$autoplay                    = $settings['carouselautoplay'] === 'true' || $settings['carouselautoplay'] === 'on' || $settings['carouselautoplay'] === 1 || $settings['carouselautoplay'] === true;
		...
		$autoscroll = $settings['autoscroll'] === 'true' || $settings['autoscroll'] === 'on' || $settings['autoscroll'] === 1 || $settings['autoscroll'] === true;
		...
		$disablelightbox = $settings['disablelightbox'] === 'true' || $settings['disablelightbox'] === 'on' || $settings['disablelightbox'] === 1 || $settings['disablelightbox'] === true;
		if ( $disablelightbox ) {
			$options_att_arr['disablelightbox'] = true;
		} else {
			$lightboxcomments = $settings['lightboxcomments'] === 'true' || $settings['lightboxcomments'] === 'on' || $settings['lightboxcomments'] === 1 || $settings['lightboxcomments'] === true;
			if ( $lightboxcomments ) {
				$options_att_arr['lightboxcomments'] = max( 1, (int) $settings['numcomments'] );
			}
		}

		$captionlinks = $settings['captionlinks'] === 'true' || $settings['captionlinks'] === 'on' || $settings['captionlinks'] === 1 || $settings['captionlinks'] === true;
		if ( $captionlinks ) {
	...
	public static function get_ajax_page_load_html() {
		...
		$js_options = array(
			...
			'placeholder' => esc_url( trailingslashit( SBI_PLUGIN_URL ) . 'img/placeholder.png' ),
			'resized_url' => esc_url( $resized_url ),
			'br_adjust'   => $br_adjust,
		);
		...
		$js_option_html .= "<script type='text/javascript' src='" . trailingslashit( SBI_PLUGIN_URL ) . 'js/sbi-scripts.min.js?ver=' . esc_js( SBIVER ) . "'></script>";
