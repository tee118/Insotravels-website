...
function sbi_get_next_post_set() {
	...
	if ( sbi_can_do_background_tasks( $post_id, $transient_name ) ) {
	...
	if ( $settings['caching_type'] === 'permanent' && empty( $settings['doingModerationMode'] ) ) {
		...
		if ( $instagram_feed->regular_cache_exists() ) {
			...
			if ( $settings['type'] === 'hashtag' || $settings['type'] === 'mixed' ) {
				$instagram_feed->add_report( 'no cache and might be hashtag feed' );

				$return = array(
					'html'          => '',
					'feedStatus'    => array( 'shouldPaginate' => false ),
	...
	$should_paginate_offset = $offset + (int) $settings['offset'];
...
function sbi_resize_posts_by_id( $ids, $transient_name, $settings, $offset = 0 ) {
	...
	if ( $instagram_feed->regular_cache_exists() ) {
		...
		if ( ! isset( $settings['cache_all'] ) || ! $settings['cache_all'] ) {
			$found_posts = array();
			$i           = 0;
			while ( isset( $cached_post_data[ $i ] ) ) {
...
function sbi_update_mod_mode_white_list() {
	if ( current_user_can( 'edit_posts' ) ) {
		...
		$white_index                = str_replace( ' ', '', $white_index );
		$feedid_chars_with_expected = preg_replace( '/[^A-Za-z0-9#_\-\/?,]/', '', str_replace( '%', '', urlencode( $white_index ) ) );
		if ( $feedid_chars_with_expected !== str_replace( '%', '', urlencode( $white_index ) ) ) {
			wp_send_json_error();
		}
		// phpcs:ignore WordPress.Security.NonceVerification
		...
		if ( $white_index === '' ) {
			...
			while ( in_array( $new_index, $current_white_names, true ) ) {
				$new_index++;
			}
		...
		if ( ! in_array( $white_index, $current_white_names, true ) ) {
...
function sbi_can_do_background_tasks( $post_id, $transient_name ) {
	...
	return $can_do_background_tasks;
}
