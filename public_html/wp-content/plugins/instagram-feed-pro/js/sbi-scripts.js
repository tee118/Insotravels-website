  ...
  (function($){
    ...
    SbiFeed.prototype = {
      ...
      afterLoadMoreClicked: function($button) {
        $button.find('.sbi_loader').css('display', 'block');
      ...
      afterNewImagesLoaded: function() {
        ...
        setTimeout(function () {
          ...
          $self.find('.sbi_loader').css('display', 'none');
      ...
      maybeRaiseSingleImageResolution: function ($item, index, forceChange) {
        ...
        if (imgSrcSet.length < 1) {
          ...
          return;
        } else if ($item.find('.sbi_link_area').length
          && $item.find('.sbi_link_area').attr('href') === window.sbi.options.placeholder.replace('placeholder.png','thumb-placeholder.png')
          || !feed.settings.consentGiven) {
          $item.find('.sbi_link_area').attr('href',imgSrcSet.d)
        }
      ...
      maybeRaiseImageResolution: function (justNew) {
        var feed = this,
          ...
          headerSelector = '.sb_instagram_header',
          forceChange = !feed.isInitialized ? true : false;

        $(feed.el).find(headerSelector).each(function (index) {
          if ($(this).find('.sbi_header_img img').length) {
            var avatarUrl = $(this).find('.sbi_header_img').attr('data-avatar-url'),
              avatarSrc = $(this).find('.sbi_header_img img').attr('src');

            if (avatarUrl.indexOf('http') > -1
              && avatarSrc.indexOf('placeholder') >= 0
              && window.sbi.feeds[ index ].settings.consentGiven){
              $(this).find('.sbi_header_img img').attr('src', avatarUrl);
            }
          }
        });
      ...
      checkConsent: function() {
        ...
        if (typeof CLI_Cookie !== "undefined") { // GDPR Cookie Consent by WebToffee
          ...
          this.settings.consentGiven = ( sbiCmplzGetCookie('cmplz_consent_status') === 'allow' || jQuery('body').hasClass('cmplz-status-marketing') );
